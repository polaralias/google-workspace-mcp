...
function registerTasksTools(server) {
    ...
    server.tool("delete_task_list", "Delete a task list.", {
        user_google_email: zod_1.z.string(),
        task_list_id: zod_1.z.string()
    ...
    server.tool("list_tasks", "List tasks in a task list.", {
        user_google_email: zod_1.z.string(),
        task_list_id: zod_1.z.string(),
        max_results: zod_1.z.number().default(20),
        page_token: zod_1.z.string().optional(),
        show_completed: zod_1.z.boolean().default(true),
        show_deleted: zod_1.z.boolean().default(false),
        show_hidden: zod_1.z.boolean().default(false)
    ...
    server.tool("create_task", "Create a new task.", {
        ...
        due: zod_1.z.string().optional().describe("RFC 3339 format"),
    ...
    server.tool("update_task", "Update a task.", {
        ...
        notes: zod_1.z.string().optional(),
        status: zod_1.z.enum(["needsAction", "completed"]).optional(),
    ...
    server.tool("clear_completed_tasks", "Clear completed tasks from a list.", {
        user_google_email: zod_1.z.string(),
        task_list_id: zod_1.z.string()
    }, async ({ user_google_email, task_list_id }) => {
        const auth = await credentialStore_1.credentialStore.getCredential(user_google_email);
