...
exports.registerSheetsTools = registerSheetsTools;
...
function parseValues(values) {
    if (typeof values === 'string') {
        try {
            ...
            throw new Error(`Invalid JSON format for values: ${e.message}`);
        }
    }
    return values;
}
function registerSheetsTools(server) {
    server.tool("list_spreadsheets", "Lists spreadsheets from Google Drive.", {
        user_google_email: zod_1.z.string().describe("The user's Google email address. Required."),
        page_size: zod_1.z.number().default(25).describe("Max spreadsheets to return."),
    ...
    server.tool("get_spreadsheet_info", "Gets information about a specific spreadsheet.", {
        user_google_email: zod_1.z.string().describe("The user's Google email address. Required."),
        spreadsheet_id: zod_1.z.string().describe("The ID of the spreadsheet.")
    ...
    server.tool("read_sheet_values", "Reads values from a specific range in a Google Sheet.", {
        user_google_email: zod_1.z.string(),
        spreadsheet_id: zod_1.z.string(),
        range_name: zod_1.z.string().default("A1:Z1000")
        ...
        try {
            ...
            if (values.length === 0)
                return { content: [{ type: "text", text: `No data found in range '${range_name}'.` }] };
    ...
    server.tool("modify_sheet_values", "Modifies values in a specific range of a Google Sheet.", {
        user_google_email: zod_1.z.string(),
        spreadsheet_id: zod_1.z.string(),
        range_name: zod_1.z.string(),
        values: zod_1.z.union([zod_1.z.string(), zod_1.z.array(zod_1.z.array(zod_1.z.any()))]).optional(),
        value_input_option: zod_1.z.enum(["RAW", "USER_ENTERED"]).default("USER_ENTERED"),
        clear_values: zod_1.z.boolean().default(false)
