...
const gmailHelpers_1 = require("./gmailHelpers");
const GMAIL_METADATA_HEADERS = ["Subject", "From", "To", "Cc", "Message-ID", "Date"];
function registerGmailTools(server) {
    server.tool("search_gmail_messages", "Searches messages in a user's Gmail account based on a query.", {
        query: zod_1.z.string().describe("The search query. Supports standard Gmail search operators."),
        user_google_email: zod_1.z.string().describe("The user's Google email address. Required."),
        page_size: zod_1.z.number().optional().default(10).describe("The maximum number of messages to return."),
    ...
    server.tool("get_gmail_message_content", "Retrieves the full content of a specific Gmail message.", {
        message_id: zod_1.z.string().describe("The unique ID of the Gmail message to retrieve."),
        user_google_email: zod_1.z.string().describe("The user's Google email address. Required.")
        ...
        try {
            ...
            const headers = (0, gmailHelpers_1.extractHeaders)(payload, GMAIL_METADATA_HEADERS);
            const { text, html } = (0, gmailHelpers_1.extractMessageBodies)(payload);
            const bodyContent = (0, gmailHelpers_1.formatBodyContent)(text, html);
            const attachments = (0, gmailHelpers_1.extractAttachments)(payload);
            ...
            if (headers["Message-ID"])
                output += `Message-ID: ${headers["Message-ID"]}\n`;
            if (headers["To"])
                output += `To:      ${headers["To"]}\n`;
            if (headers["Cc"])
                output += `Cc:      ${headers["Cc"]}\n`;
    ...
    server.tool("get_gmail_messages_content_batch", "Retrieves content of multiple Gmail messages.", {
        message_ids: zod_1.z.array(zod_1.z.string()).describe("List of Gmail message IDs to retrieve (max 25)."),
        user_google_email: zod_1.z.string().describe("The user's Google email address. Required."),
        format: zod_1.z.enum(["full", "metadata"]).optional().default("full").describe("Message format.")
    ...
    server.tool("get_gmail_attachment_content", "Downloads the content of a specific email attachment.", {
