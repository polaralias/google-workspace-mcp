...
async function listConnections() {
    const { rows } = await (0, db_1.query)('SELECT id, name, created_at FROM connections ORDER BY created_at DESC');
    if (rows.length === 0) {
...
async function listApiKeys() {
    const { rows } = await (0, db_1.query)('SELECT id, key_hash, created_at, revoked_at FROM api_keys ORDER BY created_at DESC');
    if (rows.length === 0) {
...
async function revokeApiKey(id) {
    const { rowCount } = await (0, db_1.query)('UPDATE api_keys SET revoked_at = NOW() WHERE id = $1 AND revoked_at IS NULL', [id]);
    if (rowCount === 0) {
